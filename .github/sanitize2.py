...
for wf_path, artifact in [
    (Path('.github/workflows/build-apk.yml'), 'Vidown_Android_v2.0.1.apk'),
    (Path('.github/workflows/build-windows.yml'), 'windows/installer/output/Vidown_Setup_v2.0.1.exe'),
]:
    s = wf_path.read_text(encoding='utf-8')
    if wf_path.name == 'build-apk.yml':
        s = re.sub(r'run: gh release upload \S+ ' + re.escape(artifact) + r' --clobber', "run: |\n          TAG=\"$(gh release list --limit 1 --json tagName --jq '.[0].tagName')\"\n          gh release upload \"$TAG\" " + artifact + " --clobber", s)
    else:
        s = re.sub(r'run: gh release upload \S+ ' + re.escape(artifact) + r' --clobber', "run: |\n          $tag = gh release list --limit 1 --json tagName --jq '.[0].tagName'\n          gh release upload $tag " + artifact + " --clobber", s)
    wf_path.write_text(s, encoding='utf-8')
