...
p = Path('app/src/main/java/wiki/cyberdefence/ytgrab/DownloadController.java')
s = p.read_text(encoding='utf-8')
start = s.index('            try {\n                YtDlpUpdater.ensureUpdated(app);')
end_marker = '                failIfStillRunning(id, first);\n            }'
end = s.index(end_marker, start) + len(end_marker)
generic = '''            try {
                YtDlpUpdater.ensureUpdated(app);
                runDownload(current, id);
                finishIfStillRunning(id);
            } catch (Exception error) {
                if (!isStillRunning(id)) return;
                failIfStillRunning(id, error);
            }'''
s = s[:start] + generic + s[end:]
s = s.replace('private void runDownload(DownloadJob current, String id, String playerClient) throws Exception {', 'private void runDownload(DownloadJob current, String id) throws Exception {')
s = s.replace('YoutubeDLRequest request = buildRequest(current, playerClient);', 'YoutubeDLRequest request = buildRequest(current);')
s = s.replace('private YoutubeDLRequest buildRequest(DownloadJob current, String playerClient) {', 'private YoutubeDLRequest buildRequest(DownloadJob current) {')
s = re.sub(r'\n\s*if \(playerClient != null\) \{\n\s*request\.addOption\("--extractor-args",.*?\n\s*\}\n', '\n', s, flags=re.S)
s = re.sub(r'\n\s*private boolean isYouTubeUrl\(String url\) \{.*?\n\s*\}\n', '\n', s, flags=re.S)
...
android_wf = Path('.github/workflows/build-apk.yml')
s = android_wf.read_text(encoding='utf-8')
s = re.sub(r'run: gh release upload \S+ (Vidown_Android_v2\.0\.1\.apk) --clobber', "run: |\n          TAG=\"$(gh release list --limit 1 --json tagName --jq '.[0].tagName')\"\n          gh release upload \"$TAG\" \\1 --clobber", s)
android_wf.write_text(s, encoding='utf-8')

windows_wf = Path('.github/workflows/build-windows.yml')
s = windows_wf.read_text(encoding='utf-8')
s = re.sub(r'run: gh release upload \S+ (windows/installer/output/Vidown_Setup_v2\.0\.1\.exe) --clobber', "run: |\n          $tag = gh release list --limit 1 --json tagName --jq '.[0].tagName'\n          gh release upload $tag \\1 --clobber", s)
windows_wf.write_text(s, encoding='utf-8')

banned = re.compile(r'youtube|youtu\.be|instagram|facebook|twitter|tiktok|vimeo|reddit|whatsapp', re.I)
offenders = []
for p in Path('.').rglob('*'):
    if not p.is_file() or '.git' in p.parts or p.as_posix() in {'.github/sanitize.py', '.github/workflows/sanitize-platform-names.yml'}:
        continue
    try:
...
if offenders:
    raise SystemExit('Platform-name references remain in: ' + ', '.join(offenders))
